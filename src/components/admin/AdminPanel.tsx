...
import { CategoryList } from './CategoryList';
// PasswordChangeForm removed - Auth is now handled by Middleware/Env
...
export function AdminPanel({
  ...
  useEffect(() => {
    // Auto-logout on inactivity logic removed (handled by cookie expiration mostly, or simplest to just keep active)
    ...
    const handleLogout = async () => {
      try {
        ...
        window.location.href = '/admin/login'; // Force redirect anyway
      }
    };
        ...
        <main className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8 py-4 sm:py-8">
          {showForm ? (
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4 sm:space-y-6">
              <TabsList className="grid w-full grid-cols-3 sm:grid-cols-3 bg-white shadow-soft border border-warm-beige h-auto">
                <TabsTrigger
                  value="dashboard"
